...
@interface SEFilterControlList ()<UIGestureRecognizerDelegate>
{
    CGPoint diffPoint;
    NSArray *titlesArr;
    float oneSlotSize;
...
@implementation SEFilterControlList
...
-(id) initWithFrame:(CGRect) frame Titles:(NSArray *) titles{
    if (self = [super initWithFrame:CGRectMake(frame.origin.x, frame.origin.y, frame.size.width, 80)]) {
        ...
        [_handler setFrame:CGRectMake(LEFT_OFFSET, 5, 60 * kScale, 45 * kScale)];
        ...
        image1 = [UIImage imageNamed:@"时间"];
        image2 = [UIImage imageNamed:@"距离"];
        image3 = [UIImage imageNamed:@"价格"];
...
-(void) setSelectedIndex:(int)index{
    ...
    [_handler setBackgroundImage:image forState:UIControlStateNormal];
